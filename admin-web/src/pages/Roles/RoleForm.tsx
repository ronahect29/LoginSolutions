...
import { type UpsertRoleDto, type Role } from "../../services/roles.service";

interface Props {
    initial?: Role;
    onCancel: () => void;
    onSave: (data: UpsertRoleDto, id?: number) => Promise<void>;
}

export function RoleForm({ initial, onCancel, onSave }: Props) {
    const [name, setName] = useState(initial?.name ?? "");
    const [description, setDescription] = useState(initial?.description ?? "");
    const [aplicacionId, setAplicacionId] = useState(initial?.aplicacion?.id ?? 0);
            ...
            <div className="flex flex-col md:flex-row gap-3">
                <div className="flex-1 flex flex-col gap-1">
                    ...
                    <select
                        className="border rounded-lg px-3 py-2 text-sm"
                        value={aplicacionId}
                        onChange={(e) => setAplicacionId(Number(e.target.value))}
                    >
                        <option value="superadmin">Administración</option>
                        <option value="admin">Marcajes Delivery</option>
                        <option value="user">Marcajes Outsourcing</option>
                    </select>
                </div>
